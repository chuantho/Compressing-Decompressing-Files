#In order to understand how the Huffman algorithm works we used the lecture
#notes provided by professor Dan and the following sites:
...
def tree_to_bytes(tree):
    ...
    for element in p_ordered_lst:
        if element.left != None or element.right != None:
                if element.left.left == None and element.left.right == None:
...
HuffmanNode(None, HuffmanNode(10, None, None), HuffmanNode(12, None, None)))
    ...
    if node_lst[root_index].r_type == 0 and node_lst[root_index].r_type == 0:
        tree = HuffmanNode(None, HuffmanNode(node_lst[root_index].l_data), HuffmanNode(node_lst[root_index].r_data))

    if node_lst[root_index].r_type == 1 and node_lst[root_index].r_type == 0:
        left = generate_tree_postorder(node_lst, node_lst[root_index - 2])
        tree = HuffmanNode(None, left, HuffmanNode(node_lst[root_index].r_data))

    if node_lst[root_index].r_type == 0 and node_lst[root_index].r_type == 1:
        right = generate_tree_postorder(node_lst, node_lst[root_index - 1])
        tree = HuffmanNode(None, HuffmanNode(node_lst[root_index].l_data, right))

    if node_lst[root_index].r_type == 1 and node_lst[root_index].r_type == 1:
        tree.right = HuffmanNode(None, HuffmanNode(node_lst[root_index - 1].l_data), HuffmanNode(node_lst[root_index - 1].r_data))
        tree.left = HuffmanNode(None, HuffmanNode(node_lst[root_index - 2].l_data), HuffmanNode(node_lst[root_index - 2].r_data))
